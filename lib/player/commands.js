...
// Public  {{{1
exports.mute = function(req, cb) {  // {{{2
/**
 * Toggle mute
 *
 ...
 */
  ...
  if (e.sval.volume.synced) {
    e.shard.post(e.volume, 'mute', req, cb);
  } else {
    cb && cb(O.error(this, 'Volume not connected'));
  }
};
 ...
 */
  ...
  return e.history.find(req, function(err, entry) {  // {{{3
    if (err) return O.log.error(err);

    if (entry.kind === O.data.schemas['media'].kinds.item) {
      return playItem(e, entry);
    }

    media = entry;
    return e.history.query('history', {filter: {
  ...
  function onQuery(err, resp) {  // {{{3
    ...
    switch (resp.length) {
    case 0:
      var trans = e.history.transaction();
      var entry = trans.add('item', {
        ident: media.getIdent({shard: e.history}),
        schema: media.shard.schema.name,
    ...
    case 1:
      return e.history.get(resp[0], function(err, entry) {
        if (err) return O.log.error(err);
        return playItem(e, entry);
      });
    }
 ...
 */
  ...
  if (e.sval.status === 'playing') {
    pos += (Date.now() - e.sval.pos.at) / 1000;
  }
...
function playLast(that) {  // {{{2
  if (that.sval.item) {
    that.history.find(that.sval.item, function(err, item) {
      if (item) {
        playItem(that, item);
      }
    });
...
function playItem(player, entry, socket) {  // {{{2
  ...
  player.setState({item: entry.getIdent()});
