...
function playItem(that, entry, socket) {  // {{{2
  ...
  if (! that.playback) {
    Ose.link.error(socket, Ose.error(that, 'MISSING_PLAYBACK', 'There is no playback entry defined.'));
    return;
  }

  if (! (entry && entry.data)) {
    Ose.link.error(socket, Ose.error(that, 'INVALID_ARGS', 'Invalid entry.', entry));
    return;
  }

  var scope = Ose.scope(entry.data.scope);
  if (! scope) {
    Ose.link.error(socket, Ose.error(that, 'SCOPE_NOT_FOUND', 'Scope was not found.', entry.data.scope));
    return;
  }

  var kind = scope.kinds[entry.data.kind];
  if (! kind) {
    Ose.link.error(socket, Ose.error(that, 'KIND_NOT_FOUND', 'Kind was not found.', entry.data.kind));
    return;
  }

  kind.playItem(that, entry, function(err) {
    if (err) {
