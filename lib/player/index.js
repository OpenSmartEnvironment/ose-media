...
exports.ddef = O.new('ose/lib/orm/object')()  // {{{2
  ...
  .map('sources')  // {{{3
    .query().parent
    .detail(9)
    .params({extendView: function(wrap, view) {
      view.tapItem = function(entry, ev) {
        view.stop(ev);

        if (entry.kind.tapMediaItem) {
          entry.kind.tapMediaItem(entry, wrap.owner.view.entry);
        } else {
...
function getTitle(item, info) {  // {{{2
  ...
  if (item) {
    ...
    if (item.dval) {
      result += O.translate(item.kind.name) + ' – ' + caption;
    }
  }
...
function updateExpanded(view) {  // {{{2
  ...
  if (view.mediaEntry) {
    list.li()
      .h3(O.translate('kind'))
      .p(O.translate(view.mediaEntry.shard.schema.name) + ' - ' + O.translate(view.mediaEntry.kind.name))
    ;
...
var updatePosDetail = O._.throttle(function(view) {  // {{{2
  ...
  if (view.entry.sval.status === 'playing') {
    val.value += (new Date().getTime() - s.at - view.entry.shard.masterTimeOffset) / 1000;
    val.aim = s.length;
