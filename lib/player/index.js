...
exports = O.init('control', 'player');

var Dvb = O.getClass('./dvb');
var Playback = O.getClass('./playback');
var Volume = O.getClass('./volume');
    ...
    .params({extendView: function(wrap, view) {  // TODO: this method must be defined on "detail" layout
      ...
      view.extend.tapItem = function(entry, ev) {
        if (entry.kind.tapMediaItem) {
          ...
          wrap.owner.view.entry.post('playItem', entry.getIdent());
        }
...
exports.homeInit = function(entry) {  // {{{2
  ...
  if (entry.dval.sources && entry.dval.sources.history) {
    entry.shard.findSibling(entry.dval.sources.history.ident, function(err, shard) {
      if (err) {
        O.log.error(err);
      } else {
        entry.history = shard;
    ...
    O.log.error(this, 'Missing history source!');
  }
...
function checkMediaItem(view, value) {  // {{{2
  ...
  return view.entry.shard.findSibling(view.entry.dval.sources.history.ident, function(err, shard) {
  ...
  function find() {
    view.history.find(view.entry.sval.item, function(err, entry) {
      if (err) {
        O.log.error(err);
        view.mediaEntry = null;
        updateInfoDetail(view);
        ...
        view.history.find(entry.dval.ident, onMediaEntry);
      }
    });
  }
