...
exports.displayLayout = function() {  // {{{2
  ...
  this.add([
    ...
    '<li class=pos><div><h3>Progress (broken Gaia component)</h3></div></li>',
  ].join(''));
...
var updateMediaInfo = O._.throttle(function(that) {  // {{{2
  ...
  if (s.pos && s.pos.length) {
    list.append('li').append('div').add([
      ...
      that.new('p').add([
        updatePosText(that, that.new('span')),
        that.new('span').text(' of ' + Math.round(s.pos.length)),
      ]),
    ]);
  }
