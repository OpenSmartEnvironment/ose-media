...
exports.displayLayout = function() {  // {{{2
  ...
  */
  ...
  Sources.each(function(source) {  // Create <li> for each registered source {{{3
    // Source 'id' is safe, checked when added.
    var src = that.append('<li><div><h3>' + source.id + '</h3></div></li>')
      .on('click', tapSource.bind(that))
    ;
    src.el.ose = src;
    src.source = source;
    src.kind = O.getKind(source);
  });
